...
namespace GOST_Control
{
    public class CheckingeContents
    {
        ...
        /// <returns></returns>
        public async Task<(bool IsValid, List<string> Errors)> CheckHeaderIndentsAsync(List<Paragraph> paragraphs, Action<string, IBrush> updateUI)
        {
            return await Task.Run(() =>
            {
                ...
                foreach (var paragraph in paragraphs)
                {
                    ...
                    if (_gost.HeaderIndentLeft.HasValue)
                    {
                        ...
                        if (Math.Abs(actualLeft - _gost.HeaderIndentLeft.Value) > 0.05)
                        {
                            errorDetails.Add($"левый отступ: {actualLeft:F2} см (требуется {_gost.HeaderIndentLeft.Value:F2} см)");
                            hasError = true;
                        }
                    }
                    ...
                    if (_gost.HeaderFirstLineIndent.HasValue)
                    {
                        ...
                        if (!string.IsNullOrEmpty(_gost.HeaderIndentOrOutdent) && _gost.HeaderIndentOrOutdent != "Нет")
                        {
                            string requiredType = _gost.HeaderIndentOrOutdent == "Выступ" ? "Выступ" : "Отступ";
                        ...
                        {
                            if (Math.Abs(currentValue.Value - _gost.HeaderFirstLineIndent.Value) > 0.05)
                            {
                                errorDetails.Add($"{currentType} первой строки: {currentValue.Value:F2} см (требуется {_gost.HeaderFirstLineIndent.Value:F2} см)");
                                hasError = true;
                            }
                        }
                        else if (!string.IsNullOrEmpty(_gost.HeaderIndentOrOutdent) && _gost.HeaderIndentOrOutdent != "Нет")
                        {
        ...
        /// <param name="updateUI"></param>
        /// <returns></returns>
        public async Task<(bool IsValid, List<string> Errors)> CheckHeaderParagraphSpacingAsync(List<Paragraph> paragraphs, Action<string, IBrush> updateUI)
        {
            return await Task.Run(() =>
            {
                ...
                foreach (var paragraph in paragraphs)
                {
                    ...
                    if (_gost.HeaderLineSpacingValue.HasValue)
                    {
                        ...
                        if (spacingSource?.Line != null)
                        {
                            ...
                            {
                                actualSpacing = double.Parse(val) / 20.0;
                    ...
                    {
                        errors.Add($"Заголовок '{paragraph.InnerText.Trim()}': {string.Join(", ", paraErrors)}");
                        hasErrors = true;
                    }
                }
        ...
        /// <param name="updateUI"></param>
        /// <returns></returns>
        public async Task<(bool IsValid, List<string> Errors)> CheckRequiredSectionsAsync(Gost gost, List<Paragraph> paragraphs, WordprocessingDocument doc, Action<string, IBrush> updateUI)
        {
            return await Task.Run(() =>
            {
                var errors = new List<string>();
                ...
                // Получение стилей заголовков (ИМЕННО КАК ТЫ НАПИСАЛ)
                var headerStyles = doc.MainDocumentPart?.StyleDefinitionsPart?.Styles.Elements<Style>().Where(s => s.StyleName?.Val?.Value?.StartsWith("Heading") == true ||
                                                                                                              s.StyleName?.Val?.Value?.StartsWith("Заголовок") == true)
                    ...
                    foreach (var paragraph in paragraphs)
                    {
                        ...
                        if (text.IndexOf(section, StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                            ...
                            foreach (var run in paragraph.Elements<Run>())
                            {
                                ...
                                if (checkFont)
                                {
                                    var font = run.RunProperties?.RunFonts?.Ascii?.Value ?? paragraphStyle?.StyleRunProperties?.RunFonts?.Ascii?.Value ?? DefaultHeaderFont;
                ...
                if (!allSectionsFound)
                    errors.AddRange(missingSections.Select(s => $"Отсутствует раздел: {s}"));
                if (!allSectionsValid)
                    errors.AddRange(invalidSections.Select(s => $"Ошибка в разделе: {s}"));
        ...
        /// <param name="updateUI"></param>
        /// <returns></returns>
        public async Task<bool> CheckAdditionalHeadersAsync(WordprocessingDocument doc, List<Paragraph> paragraphs, Gost gost, Action<string, IBrush> updateUI)
        {
            return await Task.Run(() =>
            {
                bool isValid = true;
